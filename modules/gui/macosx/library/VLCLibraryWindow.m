...
@property BOOL pendingUserTriggeredPlaybackVideoPresentation;
...
- (void)playerCurrentMediaItemChanged:(NSNotification *)notification
{
    ...
    [self resolvePendingUserTriggeredPlaybackVideoPresentationForController:controller];
...
{
    VLCPlayerController * const controller = notification.object;
    if ([controller isKindOfClass:VLCPlayerController.class]) {
        [self resolvePendingUserTriggeredPlaybackVideoPresentationForController:controller];
    }
...
- (void)prepareForUserTriggeredPlaybackFromBrowse
{
    self.pendingUserTriggeredPlaybackVideoPresentation =
        self.librarySegmentType == VLCLibraryBrowseSegmentType &&
        !self.embeddedVideoPlaybackActive &&
        _acquiredVideoView != nil;
}

- (void)resolvePendingUserTriggeredPlaybackVideoPresentationForController:(VLCPlayerController *)controller
{
    if (!self.pendingUserTriggeredPlaybackVideoPresentation ||
        self.embeddedVideoPlaybackActive ||
        _acquiredVideoView == nil) {
        return;
    }

    NSURL * const currentItemUrl = controller.URLOfCurrentMediaItem;
    VLCMediaLibraryMediaItem * const mediaItem =
        currentItemUrl != nil ? [VLCMediaLibraryMediaItem mediaItemForURL:currentItemUrl] : nil;
    if (mediaItem != nil) {
        if (mediaItem.mediaType == VLC_ML_MEDIA_TYPE_AUDIO) {
            self.pendingUserTriggeredPlaybackVideoPresentation = NO;
            return;
        }
    ...
    const BOOL hasKnownVideoTracks = controller.videoTracks.count > 0;
    const BOOL hasKnownAudioTracks = controller.audioTracks.count > 0;

    if (hasKnownVideoTracks) {
        [self enableVideoPlaybackAppearance];
        self.pendingUserTriggeredPlaybackVideoPresentation = NO;
    } else if (hasKnownAudioTracks) {
        self.pendingUserTriggeredPlaybackVideoPresentation = NO;
    }
}
...
- (void)enableVideoPlaybackAppearance
{
    ...
    /*
     * Track metadata can briefly lag behind vout creation when switching from the
     * browse view to another video. If we already have an active video output for
     * the library window, trust that over the track-selection snapshot so the main
     * video view is restored for the next item.
     */
