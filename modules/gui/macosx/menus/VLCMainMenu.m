...
static inline BOOL VLCConfigOptionExists(const char *name)
{
...
- (void)awakeFromNib
{
    ...
    if (psz_config) {
        if (!strstr(psz_config, "postproc") || !VLCConfigOptionExists("postproc-q"))
    ...
    const BOOL subtitleTextColorSupported = VLCConfigOptionExists("freetype-color");
    [_subtitle_textcolor setEnabled:subtitleTextColorSupported];
    [_subtitle_textcolorMenu removeAllItems];
    if (subtitleTextColorSupported) {
        [self setupMenu:_subtitle_textcolorMenu
            withIntList:"freetype-color"
            andSelector:@selector(switchSubtitleOption:)];
    }

    const BOOL subtitleBackgroundOpacitySupported = VLCConfigOptionExists("freetype-background-opacity");
    [_subtitle_bgopacity setEnabled:subtitleBackgroundOpacitySupported];
    if (subtitleBackgroundOpacitySupported) {
        [_subtitle_bgopacity_sld setIntegerValue:config_GetInt("freetype-background-opacity")];
    } else {
        [_subtitle_bgopacity_sld setIntegerValue:0];
        [_subtitle_bgopacity_sld setEnabled:NO];
    }

    const BOOL subtitleBackgroundColorSupported = VLCConfigOptionExists("freetype-background-color");
    [_subtitle_bgcolor setEnabled:subtitleBackgroundColorSupported];
    [_subtitle_bgcolorMenu removeAllItems];
    if (subtitleBackgroundColorSupported) {
        [self setupMenu:_subtitle_bgcolorMenu
            withIntList:"freetype-background-color"
            andSelector:@selector(switchSubtitleOption:)];
    }

    const BOOL subtitleOutlineThicknessSupported = VLCConfigOptionExists("freetype-outline-thickness");
    [_subtitle_outlinethickness setEnabled:subtitleOutlineThicknessSupported];
    [_subtitle_outlinethicknessMenu removeAllItems];
    if (subtitleOutlineThicknessSupported) {
        [self setupMenu:_subtitle_outlinethicknessMenu
            withIntList:"freetype-outline-thickness"
            andSelector:@selector(switchSubtitleOption:)];
    }
...
- (void)updateSubtitlesMenu:(NSNotification *)notification
{
    ...
    const BOOL subtitleBackgroundOpacitySupported =
        VLCConfigOptionExists("freetype-background-opacity");
    ...
    self.subtitle_bgopacityLabel_gray.hidden = enabled && subtitleBackgroundOpacitySupported;
    self.subtitle_bgopacityLabel.hidden = !enabled || !subtitleBackgroundOpacitySupported;
    self.subtitle_bgopacity_sld.enabled = enabled && subtitleBackgroundOpacitySupported;
}
...
{
    if ([sender tag] != -1 && !VLCConfigOptionExists("postproc-q")) {
        return;
    }
...
{
    if (!VLCConfigOptionExists("freetype-background-opacity")) {
        return;
    }
...
- (IBAction)openFileInNewInstance:(id)sender
{
    ...
    openPanel.title = _NS("Open File in New VLC Instance");
    openPanel.prompt = _NS("Open");
    ...
    alert.alertStyle = NSAlertStyleWarning;
    alert.messageText = _NS("Unable to Open New VLC Instance");
    alert.informativeText = launchError.localizedDescription ?: _NS("The selected files could not be opened in a separate VLC process.");
    [alert runModal];
}
